...
- 连接标识符是一个由客户端选择的64比特长度的统计随机数，用来标识该条连接。QUIC连接设计的初衷就是为了客户端在漫游状态，使用IP四元组(源IP、目的IP、源端口、目的端口)不足以标识这条连接时，依然可以维持连接。对于每一个传播方向，当四元组可以明确的标识这条连接时，连接标识符可能被忽略。
...
一个版本协商包仅由服务端发出。版本协商包以8位的公共标志位和64位的连接标识符开始。公共标志位必须设置PUBLIC\_FLAG\_VERSION标志位且指明64位的连接标识符。版本协商包的其余部分为服务端支持的一组4字节的版本号列表。  
...
一个公共重置包由8位的公共标志位和64位连接标识符开始。公共标志位必须设置PUBLIC\_FLAG\_RESET且指明64位的连接标识符。如果这是PRST标签的一个加密握手信息，则重置包的其余部分需要被编码(见[[QUIC-CRYPTO](https://docs.google.com/document/d/1g5nIXAIkN_Y-7XJW5K45IblHd_L2f5LTaDUDwvZ5L6g/edit)])。   
...
普通包是经过验证和加密的。包头经过验证但不是加密的，以第一个帧开始的部分是加密的。紧随头部之后的是AEAD(验证加密的关联数据)数据。数据必须依序解密以保证内容可以被正确解释。解密之后明文由按顺序排好的帧组成。  

(TODO:加解密的方法需要文本化描述。)

### 4.3.1 数据帧 ###

数据帧数据由一系列类型前缀的数据分片所组成。帧类型在本文档后面章节介绍，这里给出一般的数据帧格式如下：
...
本章概念性地描述了QUIC连接中的用于传输的流的使用。多种本章提及的帧信息主要描述了帧类型和格式。
